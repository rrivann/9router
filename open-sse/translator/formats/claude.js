...
export function lastCacheableToolIndex(tools) {
  ...
  for (let i = tools.length - 1; i >= 0; i--) {
    if (!tools[i]?.defer_loading) return i;
  }
  return -1;
}
...
export function normalizeClaudePassthrough(body, model = "") {
  ...
  // Also drop server_tool_use blocks carrying a foreign (non-srvtoolu_) id.
...
export function prepareClaudeRequest(body, provider = null) {
  ...
  // Budget guard: Anthropic accepts at most 4 blocks carrying cache_control.
  // The re-anchor above pinned the last system block and last cacheable tool,
  // so at most 2 slots remain. At >= 4 markers the client has spent the rest
  // of the budget and every remaining marker is itself a valid breakpoint —
  // re-anchoring the tail could only exceed 4, so trim instead.
