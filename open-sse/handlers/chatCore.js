...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, sourceFormatOverride, providerThinking }) {
  ...
  if (!executor.noAuth && (providerResponse.status === HTTP_STATUS.UNAUTHORIZED || providerResponse.status === HTTP_STATUS.FORBIDDEN)) {
    try {
      // Mutate credentials after each successful refresh: rotating-refresh-token
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, filtersApplied, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqTag, log, credentials, providerHeaders, providerUrl, proxyOptions };
