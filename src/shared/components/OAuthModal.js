...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta }) {
  ...
  const startPolling = useCallback(async (deviceCode, codeVerifier, interval, extraData, deadlineMs) => {
    ...
    // earlier than the device code itself. Default 120s matches the prior
    // behavior for providers that don't surface a value.
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      const deviceCodeProviders = ["codebuddy-cn"];
      ...
      // Authorization code flow (only reachable if a non-device-code provider is
      // ever wired in this fork — CodeBuddy CN uses device_code).
